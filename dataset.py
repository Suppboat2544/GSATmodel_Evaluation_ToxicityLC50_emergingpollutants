...
class MoleculeDataset(Dataset):
    ...
    def _build_conformer_graphs(self, mol, atom_feats, idx):
        ...
        if idx not in self.conformers:
            # Generate fallback single conformer if missing  
            ...
            try:
                ...
                if result == 0:  # Success
                    ...
                    conformers = [conf.GetPositions()]
                else:
                    # Embedding failed, create dummy 3D coordinates
                    N = mol.GetNumAtoms()
                    dummy_coords = np.random.rand(N, 3) * 5.0  # Random 3D positions
                    conformers = [dummy_coords]
            except Exception:
                # If all else fails, create dummy coordinates
                N = mol.GetNumAtoms()
                dummy_coords = np.random.rand(N, 3) * 5.0
                conformers = [dummy_coords]
        else:
