...
def compute_and_cache_conformers(smiles, cache=CONFORMERS_CACHE,
    ...
    if cache and os.path.exists(cache):
        ...
        if smiles_indices is not None:
            # Load only the conformers for the specified indices
            return [npz[f'arr_{i}'] if f'arr_{i}' in npz else None for i in smiles_indices]
        else:
            # Try to load conformers for all SMILES
            result = []
            for i in range(len(smiles)):
                if f'arr_{i}' in npz:
                    result.append(npz[f'arr_{i}'])
                else:
                    # Generate conformer if not in cache
                    result.append(None)
            return result
